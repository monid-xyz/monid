...
import { connectedAccountAtom, nameAtom, targetAtom, venomProviderAtom } from 'core/atoms';
...
export default function TargetAddress({ nftAddress }: Props) {
  ...
  const account = useAtomValue(connectedAccountAtom);
        ...
        <AccordionItem border={0} borderRadius={0} width={'100%'}>
          ...
          <AccordionPanel
            ...
            {target !==  '' ? <Link href={VENOMSCAN_NFT + target} target="_blank">
                  ...
                  </Link> : <Text>point your domain to a monad wallet</Text>}
            <WalletInput
              setValue={_setTarget}
              title={'Venom'}
              value={_target}
