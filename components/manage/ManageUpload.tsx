...
import { fleekSdk } from "components/venomConnect";
    ...
    if (e) {
      try {
        ...
        mimetypes.includes("pdf")
          ? setUrl(IPFS_IO_URL + uri.pin.cid)
          : setImage(IPFS_IO_URL + uri.pin.cid);

        setImages((imgs) =>
          imgs && imgs.length > 0 ? [uri.pin.cid, ...imgs] : [uri.pin.cid]
        );
      } catch (error) {
        toast({
          status: "warning",
