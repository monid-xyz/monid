...
const ManagePage: NextPage = () => {
  ...
  async function saveVid(_jsonHash: string) {
    if (!isConnected) {
      toast({
        ...
        description: 'please connect your monad wallet',
        isClosable: true,
      });
      return;
    }
    ...
    if (provider.isInitialized) {
      ...
      toast({
        status: 'loading',
        title: 'saving MONID profile to the blockchain',
  ...
  useEffect(() => {
    async function getProfileJson() {
      if (connectedAccount && isConnected && provider) {
        try {
          ...
          const nftJson = {info : { owner : '' , manager : ''}, target: '', hash: '', avatar : '', name: ''};
          //console.log(nftJson);
          if (
    ...
    getProfileJson();
  }, [connectedAccount, network, nftContract, nftAddress]);
  ...
  return (
    <>
      <Head>
        ...
        <link
          rel="icon"
          href={json && !isLoading && json.avatar !== '' ? json.avatar : '/logos/monidxyz.svg'}
        />
      </Head>
                        ...
                        <Flex gap={2} justify={'stretch'}>
                          ...
                          <LightMode>
                            <Button
                              ...
                              bgGradient={
                                colorMode === 'light'
                                  ? 'linear(to-r, var(--base0), var(--bluevenom2))'
                                  : 'linear(to-r, var(--base0), var(--bluevenom2))'
                              }
